...
class Carriage
{
  private:
		...
		long prevTime;
		long TIMETOWAIT = 5000; // time to wait before moving from FOLLOW to STANDBY
  ...
  public:
    ...
    void init( Adafruit_DotStar *strip, int halfMask );
    void setPos( float _normalized_pos, bool _bFlip );
		void update();
		void debug();
